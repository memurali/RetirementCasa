<?php
namespace App\Controller;use Cake\Routing\Router;
...
{ 
	...
	{				
	/*$response = $this->response->withStatus(404);		return $response;*/	
	}
		...
		if($this->request->is('post'))
		{
			if($this->request->is('ajax'))
			{
				...
				else
				{
					...
					{						
						//$_SESSION['seo'] = '';
						if(($_POST['index_search']=='') && ($_POST['tag_select']==''))
						{
							$_SESSION['tag'] = '';			
							$_SESSION['filter'] = 'all';
							$_SESSION['match_tag'] = '';
						...
						{														
							...
							$check_tag_qry = 'SELECT DISTINCT(SEO_Tag) FROM `tblclassification` 	
												...
												SEO_Tag = "'.strtolower(addslashes($str)).'"';
		...
		{	
			...
			{				
				...
				//{
					...
					{
						$_SESSION['tag'] = strtolower($parameters['param']);
					}
				//}
				if($_SESSION['filter']=='')
				{
				...
				$responsearr = parent::searcharticle();
				if(count($responsearr[0])==0)
				{				
					$this->redirect(['controller' => 'users','action' => 'notFound']);	
				}

			}
			/****   url without page number and tag ***/
			else if($page_url=='')
			{								
				$_SESSION['sorting'] = '';
			...
			{	
				/****   url with tag and extra string only***/
				if(($parameters['param']!='') && (!is_numeric($page_url)))				
				{					
					$this->redirect(['controller' => 'users','action' => 'notFound']);									
				}	
				...
				{	
					...
					$_SESSION['tag'] =strtolower($parameters['pass'][0]);					
					$responsearr = parent::searcharticle();					
	...
	public function adminDashboardStaging()
	{
		...
		else
		{
			...
			if($this->request->is('ajax'))
			{
				...
				if($_POST['action']=='save_article_tag')
				{
					parse_str($_POST['data'], $formval);
					$sel_art_arr = parent::save_article_tag($formval);
				...
				else if($_POST['action']=='edit_changes')
				{
					...
					if($action=='save' || $action=='re_crawl' || $action=='delete' || $action=='prepublished' || $action=='published' || $action=='complete')
					{
			...
			if($this->request->is('post'))
			{
				...
				{ 
					...
					else
					{
						...
						if($_POST['action']=='article_search')
						{
							$_SESSION['art_srch_live'] = $_POST['search'];
							...
							$arr_val = rtrim($arr,'|');
							$_SESSION['live_art_title'] = $arr_val;
